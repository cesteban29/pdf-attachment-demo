...
const logger = initLogger({
    projectName: "pdfs-attachment-demo",
...
const processPdf = wrapTraced(async (pdfFile: { filename: string; url: string }) => {
    ...
    const tempPath = path.join(tempDir, pdfFile.filename);
    fs.writeFileSync(tempPath, pdfData);

    try {
        // Create a Blob object with name and lastModified
        ...
        // Create chat completion with file_id
        ...
        console.error(`Error processing ${pdfFile.filename}:`, error);
    } finally {
        // Clean up temp file
        try {
